...
async function findAuthUserIdByEmail(
  ...
  if (matchingUserRow?.user_id) {
    ...
    if (
      !authLookupError &&
      authUser?.user?.email?.trim().toLowerCase() === email
    ) {
  ...
  try {
    ...
    if (updErr) {
      ...
      if (errAny.code === "weak_password") {
        return new Response(
          JSON.stringify({
            error: "weak_password",
            message:
              "Password must meet the security requirements. Please choose a stronger password.",
          }),
          {
            status: 200,
            headers: { ...cors, "Content-Type": "application/json" },
      ...
      return new Response(
        JSON.stringify({
          error: "update_failed",
          message: errAny.message || "Could not update password.",
        }),
        {
          status: 500,
          headers: { ...cors, "Content-Type": "application/json" },
    ...
    return new Response(
      JSON.stringify({ success: true, updatedUserId: userId }),
      {
        status: 200,
        headers: { ...cors, "Content-Type": "application/json" },
      },
